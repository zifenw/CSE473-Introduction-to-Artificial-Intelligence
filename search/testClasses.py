...
class HoursWorkedTest(TestCase):

    def __init__(self, question, testDict):
        super(HoursWorkedTest, self).__init__(question, testDict)

    def execute(self, grades, moduleDict, solutionDict):
        studentSolution = moduleDict['analysis'].question0()
        studentSolution = str(studentSolution).strip().lower()
        if studentSolution.isdigit() and int(studentSolution) > 0 :
            return self.testPass(grades)
        else:
            self.addMessage("Hours worked must be an integer greater than zero.")
