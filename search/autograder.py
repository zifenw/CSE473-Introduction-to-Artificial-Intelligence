...
from util import loadModuleFile, loadStudentCode
...
def readCommand(argv):
    ...
    parser.add_option('--fail-if-not-score',
                    dest = 'failIfNotScore',
                    action = None,
                    help = 'If set, fail if the achieved score is not equal to the score passed.')
...
def evaluate(generateSolutions, testRoot, moduleDict, exceptionMap=projectParams.ERROR_HINT_MAP,
    ...
    for q in test_subdirs:
        ...
        for t in sorted(tests):
            ...
            if testDict.get("disabled", "false").lower() == "true":
                continue
            testDict['test_out_file'] = test_out_file
            if hasattr(projectTestClasses, testDict['class']):
                testClass = getattr(projectTestClasses, testDict['class'])
            else:
                testClass = getattr(testClasses, testDict['class'])
            testCase = testClass(question, testDict)
...
if __name__ == '__main__':
    ...
    moduleDict = loadStudentCode(options.studentCode, options.codeRoot, options.testCaseCode)

    if options.runTest != None:
        runTest(options.runTest, moduleDict, printTestCase=options.printTestCase,
                display=getDisplay(True, options))
    else:
        if options is not None and options.failIfNotScore:
            import io
            from contextlib import redirect_stdout
            trap = io.StringIO()
            with redirect_stdout(trap):
                points = evaluate(options.generateSolutions, options.testRoot, moduleDict,
                                  gsOutput=options.gsOutput,
                                  edxOutput=options.edxOutput, muteOutput=options.muteOutput, printTestCase=options.printTestCase,
                                  questionToGrade=options.gradeQuestion, display=getDisplay(options.gradeQuestion != None, options))
            if points.totalCount() != int(options.failIfNotScore):
                print("Failed. Points achieved: %d, score desired: %s" % (points.totalCount(), options.failIfNotScore))
                sys.exit(1)
        else: 
