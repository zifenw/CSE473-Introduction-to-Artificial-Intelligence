...
class Counter(dict):
    ...
    def sortedKeys(self):
        ...
        dictItems = list(self.items())
        sortedItems = sorted(dictItems, key=lambda x: x[1], reverse=True)
        return [keyVal[0] for keyVal in sortedItems]
...
def loadWorkingFile(moduleName):
    return loadModuleFile(moduleName, os.path.join(os.getcwd(), moduleName + ".py"))

def loadStudentCode(studentCode=projectParams.STUDENT_CODE_DEFAULT, codeRoot=os.getcwd(), testCase=projectParams.PROJECT_TEST_CLASSES):
    codePaths = studentCode.split(',')
    moduleDict = {}
    for cp in codePaths:
        moduleName = re.match(r'.*?([^/]*)\.py', cp).group(1)
        moduleDict[moduleName] = loadModuleFile(moduleName, os.path.join(codeRoot, cp))
    moduleName = re.match(r'.*?([^/]*)\.py', testCase).group(1)
    moduleDict['projectTestClasses'] = loadModuleFile(moduleName, os.path.join(codeRoot, testCase))
